...
/**
 * Set/Get the event resources.
 */
this.resources = function (resources) {
	if (!resources) {
		return data.resources;
	}
	data.resources = resources.toString();
	return this;
}
	...
	this.generate = function(calendar) {
		...
		// COMMENT
		if (data.comment) {
			g+= 'COMMENT:' + tools.escape(data.comment) + '\r\n';
		}

		// CATEGORIES
		if (data.categories) {
			g+= 'CATEGORIES:' + tools.escape(data.categories) + '\r\n';
		}

		// RESOURCES
		if (data.resources) {
			g+= 'RESOURCES:' + tools.escape(data.resources) + '\r\n';
		}
	...
	for(i in _data) {
		if(_data.hasOwnProperty(i) && ['id', 'uid', 'start', 'end', 'stamp', 'timestamp',
		'allDay', 'floating', 'repeating', 'summary', 'location', 'description',
		'organizer', 'attendees', 'alarms', 'method', 'status', 'url', 'resources',
		'categories', 'comment'].indexOf(i) > -1) {
			this[i](_data[i]);
		}
	}
};
